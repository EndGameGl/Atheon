...
public class DestinyProfileDbModel
{
	...
	)
	{
		...
		var dbModel = new DestinyProfileDbModel()
		{
			...
			MinutesPlayedTotal = destinyProfileResponse.Characters.Data.Sum(x =>
				x.Value.MinutesPlayedTotal
			),
			...
			SecondaryComponentsMintedTimestamp =
				destinyProfileResponse.SecondaryComponentsMintedTimestamp,
			CurrentGuardianRank = destinyProfileResponse.Profile.Data.CurrentGuardianRank.Select(
				x => x.RankNumber
			),
		...
		{
			var characterActivities = destinyProfileResponse.CharacterActivities.Data[
				lastPlayedCharacterId.Value
			];
			dbModel.CurrentActivityData = new PlayerActivityData()
			{
				ActivityHash = characterActivities.CurrentActivity.Hash,
				ActivityModeHashes = characterActivities
					.CurrentActivityModes.Select(x => x.Hash.GetValueOrDefault())
					.Distinct()
					.ToList(),
	...
	{
		foreach (
			var (collectibleHash, collectibleState) in destinyProfileResponse
				.ProfileCollectibles
				.Data
				.Collectibles
		)
		...
		foreach (
			var (characterId, collectibles) in destinyProfileResponse.CharacterCollectibles.Data
		)
		{
	...
	{
		foreach (
			var (recordHash, recordComponent) in destinyProfileResponse.ProfileRecords.Data.Records
		)
		{
			var recordDbModel = new DestinyRecordDbModel()
			{
				...
				Objectives =
					recordComponent.Objectives.Count > 0
						? ConvertToDbObjectives(recordComponent.Objectives)
						: null,
				IntervalObjectives =
					recordComponent.IntervalObjectives.Count > 0
						? ConvertToDbObjectives(recordComponent.IntervalObjectives)
						: null,
			};
		...
		{
			var recordComponent =
				destinyProfileResponse.CharacterRecords.Data.GetOptimalRecordAcrossCharacters(
					recordHash
				);
	...
	)
	{
		...
		{
			var progressionData =
				destinyProfileResponse.CharacterProgressions.Data.GetMostCompletedProgressionAcrossCharacters(
			...
			Progressions.Add(
				progressionHash.Hash.Value,
				new DestinyProgressionDbModel(progressionData)
			);
		}
	}

	private void FillMetrics(DestinyProfileResponse destinyProfileResponse)
	{
	...
	)
	{
		...
		{
			if (
				!destinyProfileResponse.Metrics.Data.Metrics.TryGetValue(
					metricHash,
	...
	private static List<DestinyObjectiveProgressDbModel> ConvertToDbObjectives(
		IEnumerable<DestinyObjectiveProgress> source
	)
	{
