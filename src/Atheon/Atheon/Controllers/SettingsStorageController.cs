...
public class SettingsStorageController : ApiResponseControllerBase
{
	...
	[Produces(typeof(ApiResponse<bool>))]
	public async Task<IActionResult> SetDestinyManifestPath(
		[FromBody] string manifestPath,
		bool reload
	)
	{
		try
	...
	public async Task<IActionResult> GetDestinyManifestPath()
	{
		try
		{
			var path = await _settingsStorage.GetOption<string>(
				SettingKeys.BungieManifestStoragePath
			);
	...
	public async Task<IActionResult> SetRescanIntervalTheshold([FromBody] int seconds)
	{
		try
		{
			await _settingsStorage.SetOption(
				SettingKeys.RescanIntervalTheshold,
