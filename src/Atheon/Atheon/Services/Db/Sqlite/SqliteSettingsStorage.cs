...
namespace Atheon.Services.Db.Sqlite
{
    public class SqliteSettingsStorage : ISettingsStorage
    {
        ...
        public async Task<T?> GetOption<T>(string key, Func<T>? defaultValue = null)
        {
            ...
            var result = await connection.QueryFirstOrDefaultAsync<string?>("SELECT Value FROM SettingsStorage WHERE Key = @Key", new { Key = key });
        ...
        public async Task SetOption<T>(string key, T value)
        {
            ...
            await connection.ExecuteAsync("INSERT INTO SettingsStorage(Key, Value) VALUES(@Key, @Value) ON CONFLICT (Key) DO UPDATE SET Value = @Value", new
            {
                Key = key,
