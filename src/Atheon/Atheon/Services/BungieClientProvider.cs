...
public class BungieClientProvider : IBungieClientProvider
{
	...
	public BungieClientProvider(ISettingsStorage settingsStorage, IGuildDb guildDb)
	{
	...
	private async Task<IBungieClient> ResolveClientInstance()
	{
		...
		Ensure
			.That(apiKey)
			.Is(
				key => key.IsNotNullOrEmpty(),
				errorMessage: "Api key for bungie.net apps can't be empty"
			);
		...
		Ensure
			.That(manifestPath)
			.Is(path => path.IsNotNullOrEmpty(), errorMessage: "Manifest path can't be empty");
	...
	{
		var languages = (await _guildDb.GetAllGuildSettings())
			.Select(x => x.DestinyManifestLocale.ConvertToBungieLocale())
			.Distinct()
			.ToList();
		...
		services.UseBungieApiClient(
			(config) =>
			{
				...
				{
					repository.IgnoreDefinitionType(
						DefinitionsEnum.DestinySackRewardItemListDefinition
					);
					...
					repository.IgnoreDefinitionType(
						DefinitionsEnum.DestinyLoadoutConstantsDefinition
					);
		...
		_bungieClientConfiguration =
			serviceProvider.GetRequiredService<IBungieClientConfiguration>();
		_providerConfiguration =
			serviceProvider.GetRequiredService<SqliteDefinitionProviderConfiguration>();
