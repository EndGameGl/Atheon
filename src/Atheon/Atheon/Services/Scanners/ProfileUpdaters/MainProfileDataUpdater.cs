...
namespace Atheon.Services.Scanners.ProfileUpdaters
{
	public class MainProfileDataUpdater : IProfileUpdater
	{
		...
		)
		{
			...
			dbProfile.Name =
				$"{userInfo.BungieGlobalDisplayName}#{userInfo.BungieGlobalDisplayNameCode:D4}";
			dbProfile.MinutesPlayedTotal = profileResponse.Characters.Data.Sum(x =>
			...
			{
				dbProfile.CurrentActivityData ??=
					new DataAccess.Models.Destiny.Profiles.PlayerActivityData();
				...
				dbProfile.CurrentActivityData.PlaylistActivityHash = charActivities
					.CurrentPlaylistActivity
					.Hash;
				dbProfile.CurrentActivityData.DateActivityStarted =
					charActivities.DateActivityStarted;
				dbProfile.CurrentActivityData.ActivityModeHash = charActivities
					.CurrentActivityMode
					.Hash;
		...
		)
		{
			...
			dbProfile.CurrentGuardianRank = profileResponse.Profile.Data.CurrentGuardianRank.Select(
				x => x.RankNumber
			);
			dbProfile.LastUpdated = DateTime.UtcNow;
			dbProfile.DateLastPlayed = profileResponse.Profile.Data.DateLastPlayed;
		...
		{
			if (
				dbProfile.CurrentGuardianRank
				!= profileResponse.Profile.Data.CurrentGuardianRank.Select(x => x.RankNumber)
			)
			{
				foreach (var guildSetting in guildSettings)
				{
					...
					_commonEvents.CustomProfileBroadcasts.Publish(
						new DestinyUserProfileCustomBroadcastDbModel()
						{
							...
							NewValue = profileResponse
								.Profile.Data.CurrentGuardianRank.Select(x => x.RankNumber)
								.ToString(),
