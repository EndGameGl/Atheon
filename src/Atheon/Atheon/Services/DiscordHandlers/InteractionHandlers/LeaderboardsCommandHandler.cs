...
public class LeaderboardsCommandHandler : LocalizedSlashCommandHandler
{
	...
	public async Task CreateLeaderboardForMetricAsync(
		[Autocomplete(typeof(DestinyMetricDefinitionAutocompleter))]
		[Summary("metric", "Metric to show")]
			string metricHashString,
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			if (!uint.TryParse(metricHashString, out var metricHash))
				return Error(
					FormatText(
						"FailedToParseMetricHashError",
						() => "Failed to parse metric hash: {0}",
						metricHashString
					)
				);
			...
			if (
				!bungieClient.TryGetDefinition<DestinyMetricDefinition>(
					metricHash,
			...
			var users = await _destinyDb.GetProfileMetricsAsync(
				metricHash,
				!metricDefinition.LowerValueIsBetter,
			...
			var embedBuilder = _embedBuilderService
				...
				.WithTitle(
					FormatText(
						...
						() => "{0} Leaderboard",
						metricDefinition.DisplayProperties.Name
					)
				)
			...
			for (int j = 0; j < clanReferences.Count; j++)
			{
				...
				var formattedData = _embedBuilderService.FormatAsStringTable<
					DestinyProfileLiteWithValue<int>,
					long
				>(
					usersOfClan.Count,
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			var users = await _destinyDb.GetGuardianRanksLeaderboardAsync(
				guildSettings.Clans.ToArray()
			);
			var clanIds = users.Select(x => x.ClanId).Distinct().ToArray();
			var clanReferences = await _destinyDb.GetClanReferencesFromIdsAsync(clanIds);
			...
			var embedBuilder = _embedBuilderService
				.GetTemplateEmbed()
				.WithTitle(
					Text("GuardianRanksLeaderboardTitle", () => "Guardian Ranks Leaderboard")
				);
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			for (int j = 0; j < clanReferences.Count; j++)
			{
				...
				var formattedData = _embedBuilderService.FormatAsStringTable<
					DestinyProfileLiteWithDoubleValues<int, int>,
					long
				>(
					usersOfClan.Count,
					Text("NoUsersFound", () => "No users"),
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			var users = await _destinyDb.GetGuardianTriumphScoreAsync(
				guildSettings.Clans.ToArray()
			);
			var clanIds = users.Select(x => x.ClanId).Distinct().ToArray();
			var clanReferences = await _destinyDb.GetClanReferencesFromIdsAsync(clanIds);
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			var users = await _destinyDb.GetTimePlayedLeaderboardAsync(
				guildSettings.Clans.ToArray()
			);
			var clanIds = users.Select(x => x.ClanId).Distinct().ToArray();
			var clanReferences = await _destinyDb.GetClanReferencesFromIdsAsync(clanIds);
	...
	public async Task CreateLeaderboardForTriumphProgress(
		[Autocomplete(typeof(DestinyLeaderboardValidRecordDefinitionAutocompleter))]
		[Summary("triumph", "Triumph")]
			string triumphHashString,
		[Summary("hide", "Whether to hide this message")] bool hide = false
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			if (!uint.TryParse(triumphHashString, out var recordHash))
				return Error(
					FormatText(
						"FailedToParseRecordHashError",
						() => "Failed to parse record hash: {0}",
						triumphHashString
					)
				);
			...
			if (
				!bungieClient.TryGetDefinition<DestinyRecordDefinition>(
					recordHash,
			...
			{
				users = await _destinyDb.GetRecordIntervalObjectiveLeaderboardAsync(
					recordHash,
					guildSettings.Clans.ToArray()
				);
			}
			...
			{
				users = await _destinyDb.GetRecordObjectiveLeaderboardAsync(
					recordHash,
					guildSettings.Clans.ToArray()
				);
			}
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			if (guildSettings is null)
				return GuildSettingsNotFound();
			var users = await _destinyDb.GetTotalTitlesLeaderboardAsync(
				guildSettings.Clans.ToArray()
			);
			var clanIds = users.Select(x => x.ClanId).Distinct().ToArray();
			var clanReferences = await _destinyDb.GetClanReferencesFromIdsAsync(clanIds);
			...
			var embedBuilder = _embedBuilderService
				.GetTemplateEmbed()
				.WithTitle(
					FormatText("TotalTitlesLeaderboardTitle", () => "Total Titles Leaderboard")
				);
	...
	public async Task CreateLeaderboardForSeasonPassLevelAsync(
		[Autocomplete(typeof(DestinySeasonPassDefinitionAutocompleter))]
		[Summary("season", "Season pass")]
			string seasonPassHashString,
		[Summary("hide", "Whether to hide this message")] bool hide = false
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			if (!uint.TryParse(seasonPassHashString, out var seasonPassHash))
				return Error(
					FormatText(
						"FailedToParseSeasonPassHashError",
						() => "Failed to parse season pass hash: {0}",
						seasonPassHashString
					)
				);

			var bungieClient = await _bungieClientProvider.GetClientAsync();
			if (
				!bungieClient.TryGetDefinition<DestinySeasonPassDefinition>(
					seasonPassHash,
					out var seasonPassDefinition,
					GuildLocale
				)
			)
			...
			var embedBuilder = _embedBuilderService
				.GetTemplateEmbed()
				.WithTitle(
					Text("SeasonPassLevelsLeaderboardTitle", () => "Season Pass Levels Leaderboard")
				)
