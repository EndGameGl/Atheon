...
public class ProfileDefinitionLookupCommandHandler : LocalizedSlashCommandHandler
{
	...
	public async Task GetUsersWithItem(
		[Autocomplete(typeof(DestinyCollectibleDefinitionAutocompleter))]
		[Summary("item", "Item to look for")]
			string collectibleHashString,
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			if (!uint.TryParse(collectibleHashString, out var collectibleHash))
				return Error(
					FormatText(
						"FailedToParseCollectibleHashError",
						() => "Failed to parse collectible hash: {0}",
						collectibleHashString
					)
				);
			...
			var (defName, defIcon) = _destinyDefinitionDataService.GetCollectibleDisplayProperties(
				colDef,
				GuildLocale
			);
	...
	public async Task GetUsersWithRecord(
		[Autocomplete(typeof(DestinyRecordDefinitionAutocompleter))]
		[Summary("triumph", "Triumph to look for")]
			string recordHashString,
		[Summary("has-completed-triumph", "Whether user has completed triumph or not")]
			bool hasCompletedTriumph,
		[Summary("hide", "Whether to hide this message")] bool hide = false
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			var embedBuilder = _embedBuilderService
				...
				.WithTitle(
					hasCompletedTriumph
						? FormatText(
							"UsersWithTriumphCount",
							() => "{0} users have completed {1}",
							users.Count,
							recordDefinition.DisplayProperties.Name
						)
						: FormatText(
							"UsersWithoutTriumphCount",
							() => "{0} users have not completed {1}",
							users.Count,
							recordDefinition.DisplayProperties.Name
						)
				);
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			if (
				!client.TryGetDefinition<DestinyCollectibleDefinition>(
					collectibleHash,
	...
	public async Task GetUserTitles(
		[Summary("title", "Title to look for")]
		[Autocomplete(typeof(DestinyTitleAutocompleter))]
			string titleRecordHashString,
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			if (
				!client.TryGetDefinition<DestinyRecordDefinition>(
					titleRecordHash,
					out var titleDefinition,
					GuildLocale
				)
			)
			...
			var embedBuilder = _embedBuilderService
				.GetTemplateEmbed()
				.WithTitle(
					FormatText("UsersWithTitleTitle", () => "Users who have {0} title", titleName)
				);
	...
	)
	{
		await ExecuteAndHandleErrors(async () =>
		{
			...
			var playersWithGameVersion = await _destinyDb.GetPlayersWithGameVersionAsync(
				gameVersion.ConvertToDestinyGameVersion(),
				hasVersion,
				guildSettings.Clans.ToArray()
			);
			...
			var embedBuilder = _embedBuilderService
				...
				.WithTitle(
					hasVersion is false
						? FormatText(
							"UsersWithoutGameVersion",
							() => "Users who don't have {0}",
							gameVersion
						)
						: FormatText(
							"UsersWithGameVersion",
							() => "Users who have {0}",
							gameVersion
						)
				);

			var gettersList = new List<Func<DestinyProfileLite, object>>() { user => user.Name };
			...
			{
				var reference = clanReferences[j];
				var usersOfClan = playersWithGameVersion
					.Where(x => x.ClanId == reference.Id)
					.ToList();
