...
public class EmbedBuilderService
{
	...
	public EmbedBuilder GetTemplateEmbed(Color? color = null)
	{
		...
		embedBuilder.WithFooter(
			"Atheon",
			"https://www.bungie.net/common/destiny2_content/icons/6d091410227eef82138a162df73065b9.png"
		);
		if (!color.HasValue)
		{
	...
	public EmbedBuilder CreateSimpleResponseEmbed(
		string title,
		string description,
		Color? color = null
	)
	...
	public Embed CreateErrorEmbed(Exception exception)
	{
		...
		embedBuilder.WithDescription(
			message.Length > 4000 ? $"{message.LimitTo(3995)}\n..." : message
		);
	...
	private void AddClanNameChangeData(
		EmbedBuilder eb,
		ClanBroadcastDbModel clanBroadcast,
		BungieLocales locale
	)
	...
	private void AddClanCallsignChangeData(
		EmbedBuilder eb,
		ClanBroadcastDbModel clanBroadcast,
		BungieLocales locale
	)
	...
	private void AddClanScanfinishedData(
		EmbedBuilder eb,
		DestinyClanDbModel destinyClanDbModel,
		BungieLocales locale
	)
	...
	{
		if (
			bungieClient.TryGetDefinition<DestinyCollectibleDefinition>(
		...
		{
			var (name, icon) = _destinyDefinitionDataService.GetCollectibleDisplayProperties(
			...
			if (
				destinyUserBroadcast.AdditionalData is not null
				&& destinyUserBroadcast.AdditionalData.TryGetValue(
					"completions",
					out var complString
				)
			)
		...
		)
		{
			...
			if (!string.IsNullOrEmpty(recordDefinition.DisplayProperties.Description))
				embedBuilder.AddField(
					Text(locale, "HowToCompleteTriumph", () => "How to complete:"),
					recordDefinition.DisplayProperties.Description
				);
		}
	}
		...
		)
		{
			...
			else
			{
				var recordTitleNode = bungieClient
					.Repository.GetAll<DestinyPresentationNodeDefinition>(locale)
					.FirstOrDefault(x =>
						x.CompletionRecord.Hash == destinyUserBroadcast.DefinitionHash
					);
				if (recordTitleNode is not null)
					embedBuilder.WithThumbnailUrl(
						recordTitleNode.DisplayProperties.Icon.AbsolutePath
					);
			}
			...
			embedBuilder.WithDescription(
				FormatText(
					...
					username,
					recordDefinition.TitleInfo.TitlesByGenderHash[
	...
	)
	{
		...
		)
		{
			...
			)
			{
				...
				{
					embedBuilder.WithThumbnailUrl(
						titleRecordDefinition.DisplayProperties.Icon.AbsolutePath
					);
				}
				else
				{
				...
				embedBuilder.WithDescription(
					FormatText(
						...
						username,
						titleRecordDefinition.TitleInfo.TitlesByGenderHash[
							DefinitionHashes.Genders.BodyType1
						],
						gildedCount
					)
	...
	)
	{
		...
		embedBuilder.WithTitle(
			Text(locale, "MultipleClansBroadcastTitle", () => "Clans Broadcast")
		);
		...
		switch (broadcastType)
		{
			...
			case ProfileBroadcastType.GildedTitle:
				...
				if (
					broadcast.AdditionalData is not null
					&& broadcast.AdditionalData.TryGetValue(
						"parentTitleHash",
						out var parentTitleHashUnparsed
					)
	...
	)
	{
		...
		)
		{
			...
			foreach (var (clanId, clanData) in clansData)
			{
				...
				embedBuilder.AddField(
					FormatText(locale, "ClanInGroup", () => "Clan: {0}", clanData.ClanName),
		...
		)
		{
			...
			foreach (var (clanId, clanData) in clansData)
			{
				...
				foreach (var broadcast in destinyUserBroadcasts)
				{
					...
					if (
						usernames.TryGetValue(broadcast.MembershipId, out var username)
						&& broadcast.AdditionalData.TryGetValue(
							"gildedCount",
							out var gildedCountUnparsed
						)
